...
def twitter_user
    user = Twitter::REST::Users.new do |config|
        config.consumer_key        = ENV["TWITTER_CONSUMER_KEY"]
        config.consumer_secret     = ENV["TWITTER_CONSUMER_SECRET"]
        config.access_token        = ENV["TWITTER_ACCESS_TOKEN"]
        config.access_token_secret = ENV["TWITTER_ACCESS_TOKEN_SECRET"]
...
def say_hello_Twitter
    ...
    selected_j = journalists.sample(5)
    for journalist in selected_j do
        login_twitter.update("#{journalist} Merci pour votre travail ! #bonjour_monde")
    end
end
...
def follow_hashtag
    login_twitter.search("#bonjour_monde", result_type: "recent").take(20).collect do |tweet|
        login_twitter.follow(tweet.user)
    end
end


def like_follow_hashtag_live
    stream_twitter.search("#bonjour_monde").collect do |tweet|
        stream_twitter.fav tweet
        stream_twitter.follow(tweet.user)
    end
end